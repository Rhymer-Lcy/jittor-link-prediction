...
df_raw = (
    pd.read_csv(tl.train_csv).drop_duplicates(subset=["src", "dst", "time"]).reset_index(drop=True)
)
...
def crossfit(Z, tag):
    ...
    for f in (0, 1):
        a = np.flatnonzero(fold != f)
        b = np.flatnonzero(fold == f)
...
tl.build_cooc(df_raw, num_entity)
base_cache = ep.build_base_cache(df_raw)
cache_mat = tl.cache_dict_to_matrix(base_cache, set(), num_entity - 1)
dst_pop_log = np.log1p(tl.dst_pop)
rpop = tl.dst_rpop_log.copy()
dst_pop_raw = tl.dst_pop.copy()
...
for beg in range(0, N, CHUNK):
    ...
    collab_rows = ep.grouped_collab(
        cache_mat, [prod_q[i][0] for i in range(beg, end)], [prod_q[i][2] for i in range(beg, end)]
    )
    for j, i in enumerate(range(beg, end)):
        ...
        f_collab = tl.rownorm(collab_rows[j].astype(np.float64))
        f_rpop = tl.rownorm(rpop[cc])
        f_icfm = np.zeros(m)
        f_icf3 = np.zeros(m)
        if len(hist_d) > 0:
...
Z2 = np.concatenate(
    [Xall.reshape(-1, 18), np.concatenate(f1max_sv)[:, None], np.concatenate(f1mean_sv)[:, None]],
    axis=1,
)
...
Z3 = np.concatenate(
    [Xall.reshape(-1, 18), np.concatenate(f2max_sv)[:, None], np.concatenate(f2mean_sv)[:, None]],
    axis=1,
)
...
print(
    f"pass3 vs pass2 top-1 disagreement: {(s3.argmax(1) != s2_all.argmax(1)).mean() * 100:.2f}%",
    flush=True,
)
