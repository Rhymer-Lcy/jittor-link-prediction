...
only byte form of it that is hash-pinned. Rebuilding the base itself from raw
competition data is a separate, longer job -- see ``docs/CURRENT_PRODUCTION.md``
and ambiguity A1 in ``docs/maintenance/repository-reorganisation-ambiguities.md``.
...
def main() -> int:
    ...
    source.add_argument("--base", type=Path, default=None,
                        help="base dataset2 score CSV; overrides --base-zip")
    source.add_argument("--base-zip", type=Path, default=DEFAULT_BASE_ZIP,
    ...
    ap.add_argument("--verify", action="store_true",
                    help="assert every anchor against configs/production.json")
    ...
    if anchors:
        check("base sha256", base_sha,
              anchors["dataset2"]["decoder_input"]["sha256"])
    else:
        print(f"  base sha256 {base_sha}")

    test = pd.read_csv(args.test)
    if anchors:
        check("test.csv sha256", sha256_file(args.test),
    ...
    census = info["census"]
    print(f"[1] {xte.STRATEGY_ID}: {info['actions']} actions, "
    ...
    if anchors:
        ...
        check("pairs collapsed to a tie", info["pairs_collapsed_to_tie"],
    ...
    if not np.array_equal(np.argsort(-reread, axis=1, kind="stable"),
