...
def namespace(**overrides):
    ...
    defaults = dict(data_root=run_all.default_data_root(), data_pack="data_A",
                    output_root=run_all.default_output_root(), log_dir=None,
                    fresh=False, plan=False, quiet=False)
...
class CommandConstruction(DataFixture):
    ...
    def test_no_torch_diagnostic_or_historical_command_is_ever_built(self):
        forbidden = ("torch", "compare_backends", "train_line.py", "train_bpr.py",
                     "canonical_run.sh", "reference", "r22_rgr", "r30_xte")
        for dataset in ("dataset1", "dataset2"):
...
class Ordering(DataFixture):
    ...
    def test_dataset1_runs_before_dataset2(self):
        ...
        self.assertEqual([c[c.index("--dataset") + 1] for c in calls],
                         ["dataset1", "dataset2"])
...
class Preflight(DataFixture):
    ...
    def test_missing_dataset1_data_fails_before_any_child(self):
        ...
        code, text = self.run_main(["--data-root", str(self.data),
    ...
    def test_plan_mode_does_not_require_the_raw_data(self):
        ...
        empty.mkdir()
        self.assertEqual(run_all.preflight(
            namespace(data_root=empty, output_root=self.out, plan=True)), [])

    def test_identical_data_and_output_root_is_refused(self):
        code, text = self.run_main(["--data-root", str(self.data),
                                    "--output-root", str(self.data)])
        self.assertEqual(code, 2)
    ...
    def test_a_missing_entrypoint_is_reported(self):
        ...
        try:
            problems = run_all.preflight(namespace(data_root=self.data,
...
class Portability(DataFixture):
    ...
    def test_runs_from_a_working_directory_outside_code(self):
        ...
        """)
        result = subprocess.run([sys.executable, "-c", script], cwd=str(elsewhere),
    ...
    def test_paths_containing_spaces_are_passed_as_single_arguments(self):
        ...
        self.assertEqual(run_all.preflight(
            namespace(data_root=spaced_data, output_root=spaced_out)), [])
...
class NoScientificLogic(unittest.TestCase):
    ...
    def test_only_the_standard_library_is_imported(self):
        ...
        self.assertEqual(imported - stdlib, set(),
                         f"non-standard-library imports: {imported - stdlib}")

    def test_no_training_or_scientific_module_is_imported(self):
        for module in ("jittor", "lightgbm", "numpy", "pandas", "scipy",
                       "canonical_pipeline", "stage_contract", "pipeline_common",
                       "train_line_jt", "train_bpr_jt", "ranker_ds1", "crf_promote"):
            self.assertNotIn(f"import {module}", self.SOURCE, module)

    def test_it_defines_no_scientific_constant(self):
        for token in ("lambdarank", "n_estimators", "learning_rate", "num_leaves",
                      "tau", "epochs =", "EMB_DIM", "SEED =", "CUT =", "0.20", "115480000"):
    ...
    def test_the_module_stays_small(self):
        ...
        code_lines = [l for l in self.SOURCE.splitlines()
