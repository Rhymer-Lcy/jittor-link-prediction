...
`reference/pytorch/train_line.py` states the contract ("The entity table stays full-size so
...
#: (path, cutoff variable) for every trainer that supports a hard time cutoff.
TRAINERS = [
    (SRC / "train_line_jt.py", "LINE_TIME_MAX"),
    (SRC / "train_bpr_jt.py", "TIME_MAX"),
    (PYTORCH_REFERENCE / "train_line.py", "LINE_TIME_MAX"),
    (PYTORCH_REFERENCE / "train_bpr.py", "TIME_MAX"),
]
...
def _statement_offsets(path: Path, cutoff: str) -> tuple[int | None, int | None]:
    ...
    entity_lines = [
        ...
        and any(
            isinstance(t, ast.Name) and t.id in ("num_entity", "full_num_entity")
    ...
    if not filter_lines or not entity_lines:
        return (
            entity_lines[0] if entity_lines else None,
...
class EntityTableIsSizedBeforeTheCutoff(unittest.TestCase):
    ...
    def test_invariant_on_a_synthetic_frame(self):
        ...
        self.assertGreater(
            before, after, "the two orderings must differ, otherwise this test proves nothing"
        )
