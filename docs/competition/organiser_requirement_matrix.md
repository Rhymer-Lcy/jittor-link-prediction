...
|---|---|---|---|
| 1.1 | Archive named `contest1_<TEAM_NAME>_003.zip` | **PARTIALLY_SATISFIED** | the team name and required final name, `contest1_皮卡丘_003.zip`, are known, and the builder now derives the archive name from a required `--team-name` argument, so no placeholder remains. No current staging tree or final organiser archive exists; the name must be confirmed on the archive actually built at freeze |
...
| 4.26 | Rendered as `提交说明文档.pdf` | **PARTIALLY_SATISFIED** | the final Chinese PDF is complete and hash-verified (`0cd48507…`, 24 pages) in the cold archive; no repository-side duplicate remains. `submission_staging/` has been cleared. `stage_package.py` now stages whatever `--document` supplies under `OFFICIAL_DOCUMENT_NAME = 提交说明文档.pdf`, so freeze must pass the cold-archive PDF to that argument and then build and audit the organiser archive |
