...
### Production ranking blends (online-validated unless noted)

- **dataset1** (66% of next interactions repeat a past partner):
  `20*own_history_count + usercf + cooc_cf` — online 0.803, saturated across
  embedding variants (multi-seed / staged / structural params all within ±0.001).
- **dataset2** (0% repeats; history masked to zero):
  `0.3*usercf + 0.45*recent_popularity + 0.7*itemcf_mean + 0.5*itemcf_top3`
  where item-CF is the cosine similarity between a candidate's embedding and
  the src's historical dst embeddings. Generated by `ensemble_predict.py`.
- Offline evaluation that tracks the online ordering: negatives drawn from the
  src's actual test candidate pools (`ensemble_predict.py --eval`). CAUTION:
  it systematically overrates recency-flavored features — time-decayed history
  (dataset1) and the sequential transition feature (dataset2) both showed
  large offline gains and lost online. Trust it only for non-temporal signals.
- Refuted directions (isolated online submissions): time-decayed history
  (0.803 -> 0.7905), co-occurrence CF on dataset2 (0.526 -> 0.505), sequential
  transition feature (0.5441 -> 0.5305), multi-seed ensemble (offline tie),
  +50 training epochs past ~70 (flat).
...
- [x] Re-run after fix #1 — far above the historical 0.424 (best total 1.3513).
